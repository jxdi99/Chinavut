...
(function () {
  ...
  let lang = "th";
  const t = (key) =>
    I18N[lang] && I18N[lang][key] ? I18N[lang][key] : I18N.en[key] || key;
  ...
  function renderPreview() {
    ...
    const usableH = sectionH - GROUND_PX - 80; // Increased top clearance to 80px to avoid label overlap and provide room for dimension labels
    ...
    let startX = (sectionW - totalW) / 2;

    // Clamp startX to keep elements within bounds
    startX = Math.max(30, Math.min(startX, sectionW - totalW - 30));
  ...
  function renderTable() {
    ...
    if (!modW) {
      if (d.cabinetW === 600) {
        modW = 300;
        modH = 168.75;
      } else if (d.cabinetW === 640) {
        modW = 320;
        modH = 160;
      } else if (d.cabinetW === 960) {
        modW = 320;
        modH = 160;
      }
    }
    ...
    const RIGHT = [
      ...
      {
        l: t("displayRes"),
        v: `${d.displayResW || 0}×${d.displayResH || 0}`,
        u: t("unitPixels"),
      },
  ...
  try {
    const stateRaw =
      sessionStorage.getItem("appState") || sessionStorage.getItem("razrState");
    if (stateRaw) {
