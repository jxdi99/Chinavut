...
(function () {
    ...
    async function fetchQueue() {
        if (!supabase) return;
        // Show projects that are in 'quotation' or 'demo' stage
        const { data, error } = await supabase
            .from('led_projects')
            .select('*')
            .in('status', ['quotation', 'demo'])
    ...
    function renderQueue() {
        ...
        if (allProjects.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--muted);">ไม่มีคิวงานทำเดโม่</td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = allProjects.map(p => `
            <tr>
                ...
                <td>
                    <a href="demo-report.html?projectId=${p.id}" class="btn btn-primary" style="font-size:0.75rem;padding:6px 10px;">🎥 บันทึกการเดโม่</a>
                </td>
            </tr>
        `).join('');
    }
    ...
    function renderHistory() {
        ...
        if (allReports.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--muted);">ยังไม่มีประวัติการเดโม่</td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = allReports.map(r => `
            <tr>
                ...
                <td>
                    ${r.doc_url ? `<a href="${r.doc_url}" target="_blank">🔗 ดูรูป/ไฟล์</a>` : '-'}
                </td>
            </tr>
        `).join('');
    }
