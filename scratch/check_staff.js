...
const { data, error } = await supabase.from('staff').select('username, password, name, nick');
if (error) {
  console.error("Error:", error);
} else {
  console.table(data.map(r => ({ username: r.username, password: r.password, nick: r.nick })));
}
